...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{../results/figures/standard_cp_thresholds}
...
The Adaptive Prediction Sets approach, originally developed for multi-class classification by \citet{romano2020classification}, constructs prediction sets by ordering labels according to predicted probability and including labels sequentially until a calibrated threshold is exceeded.
We adapt this method to the multi-label setting following the approach of \citet{stutz2021learning}.
...
The Chow-Liu algorithm \citep{chow1968approximating} finds the maximum spanning tree that best approximates the joint distribution of labels using pairwise mutual information as edge weights.
For labels $j$ and $k$, the mutual information is:

\begin{multline}
I(Y^{(j)}; Y^{(k)}) = \sum_{y_j, y_k \in \{0,1\}} P(Y^{(j)}=y_j, Y^{(k)}=y_k) \\
\times \log \frac{P(Y^{(j)}=y_j, Y^{(k)}=y_k)}{P(Y^{(j)}=y_j)P(Y^{(k)}=y_k)}\label{eq:equation10}
\end{multline}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{../results/figures/chow_liu_tree}
...
\item \textbf{vs. Tree-based methods} (CQioC): While tree-based approaches model pairwise dependencies, they use mutual information which is inherently symmetric: $I(Y^{(j)}; Y^{(k)}) = I(Y^{(k)}; Y^{(j)})$. CWCS uses conditional probabilities $M_{jk} \neq M_{kj}$, capturing directional relationships like ``infiltration predicts pneumonia more strongly than pneumonia predicts infiltration.''

\item \textbf{vs. Nested set methods} (CDioC/APS): These methods order labels by confidence but do not explicitly model label dependencies. CWCS combines probability-based ordering with co-occurrence weighting, allowing clinical structure to inform inclusion decisions beyond mere predicted probability.
\end{itemize}
